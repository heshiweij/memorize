...
public class CoverFlowLayoutManger extends RecyclerView.LayoutManager {
    ...
    public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler,
        ...
        int travel = (int) (0.5*dx);
        if ((int) (0.5*dx) + mOffsetAll < 0) {
            travel = -mOffsetAll;
        } else if ((int) (0.5*dx) + mOffsetAll > getMaxOffset()){
            travel = (int) (getMaxOffset() - mOffsetAll);
        }
        mOffsetAll += travel; //累计偏移量
        layoutItems(recycler, state, (int) (0.5*dx) > 0 ? SCROLL_RIGHT : SCROLL_LEFT);
        return travel;
