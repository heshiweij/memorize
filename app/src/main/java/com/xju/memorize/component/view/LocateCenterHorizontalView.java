package com.xju.memorize.component.view;
...
public class LocateCenterHorizontalView extends RecyclerView {
   ...
   //当要调用moveToPosition()方法时要先记录已经移动了多少位置
    ...
    //置初始化时选中的位置,该方法必须在{@link LocateCenterHorizontalView#setAdapter(android.support.v7.widget.RecyclerView.Adapter) }之前调用
   //initPos 初始位置，如果位置超过了item的数量则默认选中最后一项item
    ...
    //itemCount 必须为奇数，否则默认会设置成小于它的最大奇数
    //置每次显示多少个item,该方法必须在{@link LocateCenterHorizontalView#setAdapter(android.support.v7.widget.RecyclerView.Adapter) }之前调用
